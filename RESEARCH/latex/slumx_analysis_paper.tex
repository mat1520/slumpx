...
\begin{enumerate}
    ...
    \item Evaluate how SlumX supports more than one architecture and also has a cross-platform ability to work across different operating systems.
    \item Recommend possible methods to mitigate the risks associated with the vulnerabilities and patterns of behavior discovered.
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    % --- TÍTULO ---
    \node[font=\large\bfseries] (title) {Security Risk Assessment};

    % Espacio entre el título y la matriz
    \node[below=0.2cm of title] (startmatrix) {};

    % --- MATRIZ 2x2 (centrada) ---
    % Fila 1
    \node[box, fill=red!50, below=0cm of startmatrix] (topleft)
        {Hardcoded\\C\&C Address\\(Single Point\\Failure)};

    \node[box, fill=yellow!50, right=0cm of topleft] (topright)
        {Plain Text\\Communication\\(Traffic\\Analysis)};

    % Fila 2
    \node[box, fill=yellow!50, below=0cm of topleft] (bottomleft)
        {Fixed Port\\Usage\\(Network\\Filtering)};

    \node[box, fill=green!50, right=0cm of bottomleft] (bottomright)
        {Predictable\\Binary Names\\(Signature\\Detection)};

    % --- LEYENDA CENTRADA ---
    \node[below=1.2cm of bottomleft] (legendstart) {};

    % Item 1
    \node[legendbox, fill=red!50, below=0.3cm of bottomleft] (l1) {};
...
\begin{center}\small Risk matrix classifying the vulnerabilities found in the SlumX implementation.\end{center}
