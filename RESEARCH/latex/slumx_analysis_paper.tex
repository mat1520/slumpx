...
% --- IoT Growth Bar Chart (English, IEEE style) ---
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
            ...
            title={Global IoT Device Growth (2016--2025)}
        ]
            % Data from industry reports (IoT Analytics, DemandSage)
            ...
            % 2025 projection in a different color
            \addplot[fill=red!60, draw=black] coordinates {
                (2025, 30.9)
            };
            \legend{Historical Data, Projection}
        \end{axis}
    \end{tikzpicture}
    \caption{IoT device growth statistics. The exponential increase correlates with the expanding attack surface and the proliferation of botnets such as SlumX.}
        \begin{center}\small IoT device growth chart: shows the projected increase in connected devices and its impact on the attack surface.\end{center}
    \label{fig:iot_stats}
\end{figure}
...
% --- Mirai-Hitori Timeline (English, IEEE style) ---
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[
        node distance=1.5cm,
        year/.style={circle, draw=blue!60, fill=blue!5, very thick, minimum size=1cm, font=\bfseries},
        event/.style={rectangle, draw=black!60, fill=gray!5, rounded corners, align=center, font=\small, inner sep=5pt},
        arrow/.style={-Latex, thick, blue!80}
    ]
        % Central timeline
        \draw[line width=2pt, gray!30] (0,0) -- (0,-10);

        % 2016 - Mirai
        \node[year] (y16) at (0,0) {2016};
        \node[event, right=1cm of y16] (e16) {\textbf{Mirai (Original)}\\Attacks on DynDNS and Krebs\\Source code released ("Anna-Senpai")};
        \draw[arrow] (y16) -- (e16);

        % 2017 - Satori
        \node[year] (y17) at (0,-2.5) {2017};
        \node[event, left=1cm of y17] (e17) {\textbf{Satori / Okiru}\\Exploits in Huawei/Realtek routers\\No brute force, exploit-based};
        \draw[arrow] (y17) -- (e17);

        % 2018 - Masuta
        \node[year] (y18) at (0,-5) {2018};
        \node[event, right=1cm of y18] (e18) {\textbf{Masuta \& OMG}\\Advanced evasion and proxies\\Modular architecture};
        \draw[arrow] (y18) -- (e18);

        % 2020-2021 - Moobot
        \node[year] (y21) at (0,-7.5) {2021};
        \node[event, left=1cm of y21] (e21) {\textbf{Moobot \& Mukanashi}\\Persistence in DVR systems\\High-value targets};
        \draw[arrow] (y21) -- (e21);

        % 2023 - Hitori
        \node[year, draw=red!80, fill=red!10] (y23) at (0,-10) {2023};
        \node[event, right=1cm of y23, draw=red, line width=1pt] (e23) {\textbf{Hitori (SlumX)}\\Multi-architecture support (12+)\\Anti-analysis and encryption techniques};
        \draw[arrow, red] (y23) -- (e23);

    \end{tikzpicture}
    \caption{Evolutionary timeline of the Mirai botnet family up to the Hitori variant.}
        \begin{center}\small Timeline: illustrates the evolution from Mirai to the Hitori (SlumX) variant.\end{center}
    \label{fig:mirai_timeline}
\end{figure}
...
While extensive research has been conducted on IoT Botnets; there still exists a major gap in knowledge regarding the particular types of technology used to develop an IoT botnet and also regarding how to identify advanced Mirai variants by examining their behaviour. Even though the Hitori family (more specifically its SlumX variant) has a highly sophisticated design, it has not yet received much academic research. There is potential for this type of system to cause disruption on a global scale and, therefore, this dissertation will attempt to address these gaps.

\section{Objectives}

\subsection{General Objective}

To explore the risk presented by IoT based Botnets and the Hitori (SlumX) Botnet variant, a detailed technical analysis of this specific botnet variant is required. The focus of this technical analysis should be on the source code of the botnet to evaluate how it operates and what its main functions are.

\subsection{Specific Objectives}

\begin{enumerate}
    \item An assessment was conducted of the types of DDoS attack techniques and how these methods are delivered.
    \item An evaluation of multi-architecture support and cross-platform support mechanisms was performed.
    \item Mitigation methods for vulnerabilities identified during this analysis were recommended.
\end{enumerate}
...
\subsection{IoT Security Fundamentals}
...
% Table: Common Security Vulnerabilities in IoT Layers
\begin{table}[htbp]
    \centering
    \caption{Common Security Vulnerabilities in IoT Layers}
    \label{tab:iot_vulns}
    \begin{tabular}{@{}llp{8cm}@{}}
        {Layer} & \textbf{Component} & \textbf{Security Weakness} \\
        \midrule
        Hardware & Debug Interfaces & Exposed UART/JTAG ports allowing direct physical access to the device\cite{mahmoud2015iot}. \\
        Firmware & Update Mechanism & Lack of signed updates and presence of hardcoded credentials or backdoors\cite{pa2015internet}. \\
        Network & Transmission & Use of clear-text protocols (Telnet, HTTP) allowing traffic interception\cite{silva2013botnets}. \\
        Application & Access Control & Insufficient input validation and weak authentication mechanisms. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsection{Overview of Botnet Design}

As the severity of cyber threats increases, the development of botnets has created a significantcybersecurity problem. The evolution of botnets is ongoing as well, with today's modern-day botnets featuring an advanced system of Command and Control that facilitates the ability for criminals to communicate with the infected devices consistently.

\subsection{Centralized vs. Decentralized Architectures}

Historically, botnets have had a centralized architecture – in this method, the compromised machines (or "bots" or "zombies") communicate directly with C\&C servers. This architecture is often effective for distributing commands, but it is vulnerable because there are single points of failure and all corrupt devices rely on one centralized system to receive commands. More advanced botnets today have moved to more resilient architectures, like peer-to-peer networks, Domain Generation Algorithms (DGA), and multi-tiered architectures.

% Figure: Comparison of Botnet Architectures
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[
        node distance=1.5cm,
        server/.style={rectangle, draw=red!80, fill=red!10, very thick, minimum size=1cm, align=center},
        bot/.style={circle, draw=blue!60, fill=blue!5, thick, minimum size=0.8cm},
        link/.style={-, thick, gray}
    ]
    % --- Left Side: Centralized ---
    \node [server] (c2) at (0,0) {\textbf{C\&C Server}};
    \node [bot] (b1) at (-1.5, -2) {Bot};
    \node [bot] (b2) at (0, -2) {Bot};
    \node [bot] (b3) at (1.5, -2) {Bot};
    \draw [link] (c2) -- (b1);
    \draw [link] (c2) -- (b2);
    \draw [link] (c2) -- (b3);
    \node at (0, -3.3) {\textbf{(A) Centralized Architecture}};
    \node [align=center, font=\small] at (0, -4) {Single Point of Failure\\Direct Command Distribution};
    % --- Right Side: Decentralized (P2P) ---
    \begin{scope}[xshift=6cm]
        \node [bot] (p1) at (0,0) {Bot};
        \node [bot] (p2) at (-1.5, -1.5) {Bot};
        \node [bot] (p3) at (1.5, -1.5) {Bot};
        \node [bot] (p4) at (0, -2.5) {Bot};
        % Mesh connections
        \draw [link] (p1) -- (p2);
        \draw [link] (p1) -- (p3);
        \draw [link] (p2) -- (p4);
        \draw [link] (p3) -- (p4);
        \draw [link] (p2) -- (p3);
        \node at (0, -3.3) {\textbf{(B) Decentralized (P2P)}};
        \node [align=center, font=\small] at (0, -4) {High Resiliency\\Peer-to-Peer Command Propagation};
    \end{scope}
    \end{tikzpicture}
    \caption{Comparison of Botnet Architectures: Centralized vs. Decentralized Models.}
        \begin{center}\small Visual comparison of centralized and decentralized botnet architectures.\end{center}
    \label{fig:botnet_arch}
\end{figure}

\subsection{C\&C Infrastructure Functions}

The C\&C infrastructure provides several critical functions of the botnet: distributing commands for conducting attacks, exfiltrating data, maintaining the health and management of the botnet, and coordinating attacks across multiple botnets.\cite{silva2013botnets}.
...
\subsection{Types of DDoS Attacks}
...
\begin{itemize}
    \item \textbf{Volumetric attacks:} Massive volumes of traffic flood into and consume all of the available bandwidth on a target connection.
    \item \textbf{Protocol attacks:} Techniques are used to take advantage of the design weaknesses within certain network protocols to consume all resources associated with a server.
    \item \textbf{Application-layer attacks:} Crafted fake requests are made against specific services and applications, which appear to be legitimate (as they are coming from the IP address of an infected system). 
\end{itemize}


\subsection{Examples of DDoS Techniques}

Volumetric attacks include UDP Floods, ICMP Floods, and Amplification Attacks where the exploited public service uses the ability to multiply the amount of attack traffic generated against the target. Protocol Attacks exploit issues associated with how a TCP connection is set up/established as well as bugs related to fragmented packet processing used through Routing Protocols (RIP, BGP). Application-Layer Attacks can include targeted attacks made against HTTP Services and DNS Services along with any applications that may include specifically crafted requests intended to consume CPU time through consuming server resources.\cite{zargar2013survey}.
...
% Additional sections for Chapters 3, 4, and 5 will follow a similar restructuring pattern.
% capitulo 3
...
% Table: Summary of Previous IoT Botnet Research Studies
\begin{table}[htbp]
    \centering
    \caption{Summary of Previous IoT Botnet Research Studies}
    \label{tab:previous_studies}
    \begin{tabular}{|p{3.5cm}|c|p{4cm}|p{5cm}|}
        \hline
        \textbf{Authors} & \textbf{Year} & \textbf{Research Focus} & \textbf{Key Findings} \\
        \hline
        Kolias et al. & 2017 & Mirai botnet analysis & Scanning algorithms, target selection, attack methods; established foundational principles of IoT botnets \\
        \hline
        Meidan et al. & 2018 & Network-based detection & N-BaIoT using deep autoencoders for detecting IoT botnet attacks \\
        \hline
        Marzano et al. & 2018 & Botnet evolution & Evolution patterns of Bashlite and Mirai IoT botnets \\
        \hline
        Silva et al. & 2013 & Botnet survey & Comprehensive survey of botnet architectures and C\&C systems \\
        \hline
        Wang et al. & 2022 & Active defense & Game-theory-based intrusion detection for cyber-physical systems \\
        \hline
        Sivanathan et al. & 2020 & Device classification & Classifying IoT devices using network traffic characteristics \\
        \hline
        Roopak et al. & 2019 & Deep learning detection & Deep learning models for cybersecurity in IoT networks \\
        \hline
        Almousa et al. & 2023 & Mirai variants & Survey of Mirai botnet evolution and variant classification \\
        \hline
        Vishwakarma \& Chhabra & 2023 & Targeted DDoS & Survey of Mirai variants and targeted DDoS attacks on IoT \\
        \hline
        Alghazzawi et al. & 2024 & Hybrid detection & Efficient hybrid deep learning model for IoT botnet detection \\
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        process/.style={rectangle, minimum width=3.5cm, minimum height=1cm, text centered, draw=black, fill=blue!5},
        arrow/.style={thick,->,>=Stealth, shorten >=1mm, shorten <=1mm}
    ]
    % Nodos principales
    \node (phase1) [startstop] {\textbf{Phase 1: Data Collection}};
    \node (phase2) [process, below=of phase1] {\textbf{Phase 2: Static Code Analysis}};
    \node (phase3) [process, below=of phase2] {\textbf{Phase 3: Threat Modeling}};
    \node (phase4) [startstop, below=of phase3, fill=green!10] {\textbf{Phase 4: Validation}};

    % Descripciones (las desplazamos a la derecha para que no queden en la trayectoria de las flechas)
    \node (p1desc) [font=\footnotesize, align=center, below=0.25cm of phase1, xshift=3.2cm] {GitHub Repository \& Telegram\\Source Code Acquisition};
    \node (p2desc) [font=\footnotesize, align=center, below=0.25cm of phase2, xshift=3.2cm] {Manual Code Review\\Structure \& Logic Assessment\\(No runtime execution)};
    \node (p3desc) [font=\footnotesize, align=center, below=0.25cm of phase3, xshift=3.2cm] {Attack Surface Mapping\\Impact Assessment};
    \node (p4desc) [font=\footnotesize, align=center, below=0.25cm of phase4, xshift=3.2cm] {Cross-referencing Findings\\with Threat Intelligence};

    % Flechas (con anchors y shorten para que queden limpias)
    \draw [arrow] (phase1.south) -- (phase2.north);
    \draw [arrow] (phase2.south) -- (phase3.north);
    \draw [arrow] (phase3.south) -- (phase4.north);
    \end{tikzpicture}
    \caption{Research Methodology Flowchart applied to the SlumX analysis.}
...
Additionally, the SlumX source code was obtained through searches in Telegram groups dedicated to sharing malware source code. These groups provided access to various repositories, including the one analyzed in this research.

% --- SlumX Source Code Repository Inventory Table ---
\begin{table}[htbp]
    \centering
    \caption{SlumX Source Code Repository Inventory}
    \label{tab:repo_inventory}
    \begin{tabularx}{\textwidth}{@{}l l X@{}}
        	{Filename} & \textbf{Size (LOC)} & \textbf{Functional Description} \\
        \midrule
        	exttt{bot.c} & 1,746 & Main client payload. Contains attack vectors, connection logic, and evasion techniques. \\
        \addlinespace
        	exttt{server.c} & 845 & Command \& Control (C2) server. Manages bot connections and broadcasts attack commands. \\
        \addlinespace
        	exttt{cc.py} & 199 & Cross-compilation script. Automates building binaries for 12+ architectures. \\
        \addlinespace
        	exttt{scankill.c} & N/A* & Scanner module responsible for identifying vulnerable devices and terminating competing malware. \\
        \addlinespace
        	exttt{Netto.py} & N/A* & Specialized exploit tool targeting Netis router vulnerabilities via UDP injection. \\
        \bottomrule
    \end{tabularx}
...
Analytically reviewed, we see that the SlumX botnet has a highly advanced and complex architecture which is Multi-component architecture which allows it to have large amounts of IoT device compromises, as well as provide a means to coordinate large scale distributed denial of service attacks on victims via the internet. In addition to this, the SlumX Botnet is also built on off of the latest Mirai Botnet, providing additional features and operational security that were not available to the original Mirai botnet code base.
The core components of the SlumX Botnet consist of the following four (4) primary components: One (1) The bot client (bot.c) that is executed once it has been compromised, Two (2) The command and control server (server.c), which is responsible for managing and operating the infrastructure of the entire botnet, Three (3) The loader system, which is responsible for delivering and executing the payloads on the infected devices, and Four (4) Various auxiliary tools that allow for the scanning and exploitation of IoT devices.
...
\begin{tikzpicture}[
    ...
    % REMOVIDA: Las lineas punteadas tapaban el cuadro verde
    % Solo dejamos la etiqueta sin el recuadro
    
    % Etiqueta rotada al lado (sin caja)
    \node [rotate=90, anchor=south, font=\small] at (-3.5, -2.5) {Multi-Architecture Support};
...
\begin{center}\small Diagram showing the overall architecture of the SlumX botnet and the interaction between its main components.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: message sent by the bot to the C\&C server, identifying architecture and version.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: counts the number of bots connected per architecture on the C\&C server.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: UDP flood attack implementation with spoofing and configurable packet size.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: function to perform TCP SYN flood attacks, saturating the target's connection table.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: attack that keeps open connections to exhaust the victim server's resources.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: generates HTTP requests with random user agents to evade detection.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: exploitation for Netis routers, executing remote commands via UDP.\end{center}
...
\begin{center}\small Flowchart illustrating the infection and propagation process of SlumX in IoT devices.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: shows how the bot hides its process by changing the visible name in the system.\end{center}
...
\end{lstlisting}
\begin{center}\small Code: function implementing persistence mechanisms to keep the bot active after reboots.\end{center}
...
\begin{center}\small Chart comparing the number of lines of code for each attack vector implemented in SlumX.\end{center}
...
In this study of the SlumX botnet, we observed a considerable level of sophistication with respect to IoT malware. As described in this report, SlumX has: (1) expanded its attack surface greatly by providing multi-architecture support, which includes support for 12 different processor architectures (MIPS, ARM, x86, and embedded); (2) incorporated advanced evasion techniques, which include process masquerading, anti-analysis functionality, and IoT-specific concealment methods; (3) shifted to a more targeted form of exploitation by using vendor-specific exploits and targeting of certain protocols; and (4) evolved from an opportunistic to a targeted form of attack.

\section{Limitations}

There are several limitations associated with this research. First, conducting static analysis which does not represent the actual running of the program is impossible. Second, a static analysis of SlumX only shows the capabilities of SlumX within a limited time frame and without consideration for other times. Third, this research was conducted without testing on actual IoT devices. Lastly, these findings cannot be applied to other botnet families.
...
The detailed study of the SlumX botnet proved the sophistication of IoT malware. In addition, it also marks a dangerous new trend in the cyber threat landscape. SlumX signifies a democratization of cyber weapons, providing ease of access to DDoS tools for a large number of threat actors, with multi-architecture support and advanced evasion being the main features.

Also, the research shows the failing of the current standards of IoT security system. Dependence on the user to change the default credential is not effective enough against auto-propagation mechanisms like those in SlumX. Manufacturers should rethink their approach to make products secure by design. They must remove hardcoded passwords and ensure all products have integrity checks, among other things.

In the end, the conclusion states that static signature-based defenses are obsolete. SlumX can change its patterns and target specific vendors. Therefore, behavioral analysis and machine learning based detection at the network edge are a must. To reduce impact due to scaling by botnets into massive distributed attacks, we must focus on the operational patterns of these botnets.
% ========== BIBLIOGRAPHY ==========
