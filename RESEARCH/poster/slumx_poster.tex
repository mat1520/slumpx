...
\institute{Universidad Internacional del Ecuador - Facultad de Ciencias Técnicas- Escuela Ciencias de la Computacion}
...
\block{}{
    \centering
    \colorbox{AccentCyan!20}{\parbox{0.95\textwidth}{\centering
    \vspace{0.5em}
...
\begin{columns}
    ...
    \block{Abstract}{
        ...
        \vspace{0.5em}
        \colorbox{AccentBlue!20}{\parbox{0.96\linewidth}{\normalsize\color{TextWhite}
    ...
    \block{Threat Model}{
        \centering
        \begin{tikzpicture}[
            stat/.style={rectangle, draw=none, fill=AccentBlue!30, text=TextWhite, minimum width=4cm, minimum height=1.8cm, align=center, rounded corners=5pt, font=\large\bfseries}
        ]
            \node[stat] (a1) at (0,0) {\Huge 12+\\\normalsize Architectures};
            \node[stat] (a2) at (5.5,0) {\Huge 7+\\\normalsize Attack Vectors};
            \node[stat] (a3) at (11,0) {\Huge 1Tbps+\\\normalsize DDoS Potential};
        \end{tikzpicture}
        
        \vspace{0.8em}
        \small\textbf{\color{AccentOrange}Vulnerabilities Found:} Buffer overflows (\texttt{sprintf}), no thread sync, hardcoded credentials.
    }
