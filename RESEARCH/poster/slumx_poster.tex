\documentclass[25pt, a0paper, portrait]{tikzposter}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage{lmodern}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{array}
\usepackage{multirow}
\usepackage{xcolor}
\usepackage{colortbl}
\usepackage{pgfplots}
\pgfplotsset{compat=1.17}
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=TextWhite,
    filecolor=TextWhite,      
    urlcolor=TextWhite,
    pdftitle={SlumX Poster},
    pdfpagemode=FullScreen,
}
\usetikzlibrary{arrows.meta, positioning, shapes.geometric, fit, calc, shadows, backgrounds}

\definecolor{DarkBackground}{HTML}{0B1120}
\definecolor{BlockBackground}{HTML}{1E293B}
\definecolor{TextWhite}{HTML}{F1F5F9}
\definecolor{AccentCyan}{HTML}{22D3EE}
\definecolor{AccentBlue}{HTML}{60A5FA}
\definecolor{AccentPurple}{HTML}{C084FC}
\definecolor{AccentGreen}{HTML}{4ADE80}
\definecolor{AccentRed}{HTML}{F87171}
\definecolor{AccentOrange}{HTML}{FB923C}

\definecolorstyle{DarkNeonStyle}{
    \definecolor{colorOne}{named}{AccentCyan}
    \definecolor{colorTwo}{named}{AccentBlue}
    \definecolor{colorThree}{named}{AccentPurple}
}{
    \colorlet{backgroundcolor}{DarkBackground}
    \colorlet{framecolor}{DarkBackground}
    \colorlet{titlefgcolor}{TextWhite}
    \colorlet{titlebgcolor}{DarkBackground}
    \colorlet{blocktitlebgcolor}{BlockBackground}
    \colorlet{blocktitlefgcolor}{AccentCyan}
    \colorlet{blockbodybgcolor}{BlockBackground}
    \colorlet{blockbodyfgcolor}{TextWhite}
    \colorlet{innerblocktitlebgcolor}{AccentBlue!20}
    \colorlet{innerblocktitlefgcolor}{TextWhite}
    \colorlet{innerblockbodybgcolor}{DarkBackground}
    \colorlet{innerblockbodyfgcolor}{TextWhite}
    \colorlet{notefgcolor}{DarkBackground}
    \colorlet{notebgcolor}{AccentCyan}
    \colorlet{noteframecolor}{AccentCyan}
}

\definetitlestyle{SimpleTitle}{
    width=\paperwidth, roundedcorners=0, linewidth=0pt, innersep=2cm,
    titletotopverticalspace=5mm, titletoblockverticalspace=8mm
}{
    \draw[draw=none, fill=titlebgcolor] (\titleposleft,\titleposbottom) rectangle (\titleposright,\titlepostop);
}

\defineblockstyle{DarkCard}{
    titlewidthscale=1, bodywidthscale=1, titleleft,
    titleoffsetx=0pt, titleoffsety=0pt, bodyoffsetx=0pt, bodyoffsety=0pt,
    bodyverticalshift=0pt, roundedcorners=10, linewidth=0pt,
    titleinnersep=15pt, bodyinnersep=15pt
}{
    \begin{scope}[drop shadow={fill=black, shadow xshift=3pt, shadow yshift=-3pt, opacity=0.6}]
        \fill[color=blockbodybgcolor, rounded corners=10pt] (blockbody.south west) rectangle (blockbody.north east);
        \ifBlockHasTitle
            \draw[color=AccentCyan, line width=2pt] ([yshift=-2pt]blocktitle.south west) -- ([yshift=-2pt]blocktitle.south east);
        \fi
    \end{scope}
}

\usecolorstyle{DarkNeonStyle}
\usetitlestyle{SimpleTitle}
\useblockstyle{DarkCard}

\makeatletter
\settitle{
    \parbox{0.98\paperwidth}{
        \raggedright
        \includegraphics[height=5cm]{university_logo.png}\\[0.8em]
        \color{titlefgcolor}
        {\bfseries \LARGE \@title}\\[0.7em]
        {\LARGE \@author}\\[0.5em]
        {\Large \@institute}
    }
}
\makeatother

\title{Analysis of IoT Botnet Structure and Behavior: A Study Based on the Hitori (SlumX) Variant}
\author{Ariel Matias Melo Pazmiño}
\institute{Universidad Internacional del Ecuador - Facultad de Ciencias Técnicas - Escuela de Ciencias de la Computación}

\begin{document}

\maketitle

% Key Discovery Headline - Mensaje Principal Memorable
\block{}{
    \centering
    \tikz[baseline=(X.base)]{
        \node[fill=BlockBackground, draw=AccentCyan, line width=3pt, rounded corners=10pt, inner sep=15pt] (X) {
            \parbox{0.9\textwidth}{\centering
            {\Huge\bfseries\color{AccentCyan} SlumX: 12 Architectures $\bullet$ 7 Attack Vectors $\bullet$ 1 Sophisticated Threat}
            \\[0.3em]
            {\Large\color{TextWhite} The Evolution from Simple Mirai Variants to Advanced Multi-Platform IoT Weaponry}
            }
        };
    }
}

\begin{columns}

    \column{0.33}

    \block{\textcolor{AccentOrange}{1.} Abstract}{
        \normalsize
        \begin{itemize}\setlength\itemsep{0.4em}
            \item Static code analysis of \textbf{SlumX botnet} (Hitori variant) demonstrates a fully functional C2 mechanism [1]
            \item Multiple methods of executing \textbf{DDoS attacks} (UDP, TCP and HTTP)
            \item Supports various device architectures: \textbf{MIPS, ARM, x86}, plus nine additional [2]
            \item Advanced \textbf{evasion techniques} specific to IoT devices
        \end{itemize}
        
        \vspace{0.5em}
        \colorbox{BlockBackground}{\parbox{0.96\linewidth}{\normalsize\color{AccentCyan}
        \textbf{Contribution:} Detailed description of latest IoT botnet generations, behaviours and mitigation strategies [5].
        }}
    }

    \block{\textcolor{AccentOrange}{2.} Research Objectives}{
        \normalsize
        \textbf{\large\color{AccentBlue}General Objective}
        \vspace{0.2em}
        
        To explore the risk presented by IoT botnets, a detailed technical analysis of the Hitori (SlumX) botnet variant is required [1].

        \vspace{0.5em}
        \textbf{\large\color{AccentBlue}Specific Objectives}
        \vspace{-0.2em}
        \begin{itemize}\setlength\itemsep{0.3em}
            \item Evaluate the source code to understand how the botnet operates and its main functions.
            \item Assess DDoS attack techniques and payload delivery methods [3].
            \item Evaluate multi-architecture and cross-platform support mechanisms.
            \item Recommend mitigation methods for identified vulnerabilities [4].
        \end{itemize}
    }

    \block{\textcolor{AccentOrange}{3.} Methodology}{
        \centering
        \renewcommand{\arraystretch}{1.5}
        \begin{tabular}{>{\color{AccentCyan}\bfseries}l >{\color{TextWhite}}l}
            \textbf{\color{AccentBlue}Phase} & \textbf{\color{AccentBlue}Description} \\
            \midrule
            Data Collection & GitHub repo \texttt{mat1520/slumpx} \\
            Static Analysis & \texttt{bot.c}, \texttt{server.c}, \texttt{cc.py} \\
            Control Flow & Execution paths mapping \\
            Threat Modeling & Attack surface assessment \\
        \end{tabular}
        
        \vspace{0.8em}
        \colorbox{BlockBackground}{\parbox{0.96\linewidth}{\small\color{AccentOrange}
        \textbf{Note:} Static analysis only - no runtime execution [2].
        }}
    }



    \block{\textcolor{AccentOrange}{4.} Threat Model}{
        \centering
        \large
        \begin{tabular}{c l}
            {\Huge\bfseries\color{AccentCyan} 12+} & {\color{TextWhite} Supported Architectures} \\[0.8em]
            {\Huge\bfseries\color{AccentPurple} 7+} & {\color{TextWhite} Attack Vectors} \\[0.8em]
            {\Huge\bfseries\color{AccentOrange} 1Tbps+} & {\color{TextWhite} DDoS Potential} \\
        \end{tabular}
        
        \vspace{1em}
        \colorbox{BlockBackground}{\parbox{0.95\linewidth}{\small\color{AccentOrange}
        \textbf{Vulnerabilities:} Buffer overflows, no thread sync, hardcoded credentials.
        }}
    }

    \block{\textcolor{AccentOrange}{5.} System Architecture}{
        \centering
        \begin{tikzpicture}[
            scale=1.1, transform shape,
            node distance=2cm and 2.5cm,
            >=Stealth,
            server/.style = {rectangle, draw=AccentBlue, fill=DarkBackground, text width=5cm, align=center, rounded corners=5pt, minimum height=2cm, font=\normalsize\bfseries, text=AccentBlue, line width=2pt, drop shadow},
            client/.style = {rectangle, draw=AccentPurple, fill=DarkBackground, text width=4.5cm, align=center, rounded corners=5pt, minimum height=1.6cm, font=\normalsize\bfseries, text=AccentPurple, line width=2pt, drop shadow},
            module/.style = {rectangle, draw=AccentCyan, fill=DarkBackground, text width=5cm, align=center, rounded corners=5pt, minimum height=1.5cm, font=\normalsize\bfseries, text=AccentCyan, line width=2pt, drop shadow},
            attack/.style = {ellipse, draw=AccentOrange, fill=DarkBackground, text width=3cm, align=center, minimum height=1.4cm, inner sep=2pt, font=\normalsize\bfseries, text=AccentOrange, line width=2pt, drop shadow},
            connection/.style = {draw, ->, line width=2pt, TextWhite}
        ]
            \node [server] (cnc) {C\&C Server\\ (server.c)};
            \node [client, below=1.8cm of cnc] (bot) {Bot Client\\ (bot.c)};
            \node [module, right=3cm of cnc] (build) {Build\\ (cc.py)};
            \node [module, below=1.8cm of build] (scanner) {Scanner\\ (scankill.c)};
            \node [attack, below=2.2cm of bot] (http) {HTTP\\ Flood};
            \node [attack, left=0.5cm of http] (tcp) {TCP\\ Flood};
            \node [attack, right=0.5cm of http] (hold) {HOLD\\ Attack};
            
            \draw [connection] (cnc) -- node[right, font=\small, text=TextWhite] {Cmd} (bot);
            \draw [connection] (build) -- node[above, font=\small, text=TextWhite] {Build} (cnc);
            \draw [connection] (scanner) -- node[above, font=\small, text=TextWhite] {Tgt} (bot);
            \draw [connection] (bot) -- (tcp);
            \draw [connection] (bot) -- (http);
            \draw [connection] (bot) -- (hold);
        \end{tikzpicture}
        
        \vspace{0.8em}
        \begin{minipage}{0.95\linewidth}
            \normalsize
            A centralized command-and-control model is employed. The server part uses TCP port 1994 to control compromised systems; the \texttt{bot.c} component is distributed among infected devices. A Python-based build system (\texttt{cc.py}) enables cross-compilation for each processor type.
        \end{minipage}
    }

    \column{0.33}

    \block{\textcolor{AccentOrange}{6.} Infection Process Flow}{
        \centering
        \begin{tikzpicture}[
            scale=1.1, transform shape,
            node distance=2cm, auto, >=Stealth,
            process/.style = {rectangle, draw=AccentBlue, fill=DarkBackground, text width=5cm, text centered, rounded corners=5pt, font=\normalsize\bfseries, text=AccentBlue, minimum height=1.5cm, line width=2pt, drop shadow},
            decision/.style = {diamond, draw=AccentOrange, fill=DarkBackground, text width=3.5cm, text centered, aspect=2, font=\normalsize\bfseries, text=AccentOrange, line width=2pt, drop shadow},
            data/.style = {ellipse, draw=AccentCyan, fill=DarkBackground, text width=3cm, text centered, font=\normalsize\bfseries, text=AccentCyan, line width=2pt, drop shadow},
            arrow/.style = {draw, ->, line width=2pt, TextWhite}
        ]
            \node [data] (start) {Target\\IP Range};
            \node [process, below=of start] (scan) {Network\\Scanning};
            \node [decision, below=of scan] (vuln) {Vuln.\\Device?};
            \node [process, below left=of vuln] (exploit) {Send\\Exploit};
            \node [process, below=of exploit] (payload) {Download\\Payload};
            \node [process, below=of payload] (execute) {Execute\\Binary};
            \node [process, below=of execute] (connect) {Connect\\C\&C};
            \node [data, right=3.5cm of vuln] (next) {Next\\Target};
            
            \draw [arrow] (start) -- (scan);
            \draw [arrow] (scan) -- (vuln);
            \draw [arrow] (vuln) -- node[below right, font=\small, text=TextWhite] {Yes} (exploit);
            \draw [arrow] (vuln) -- node[above, font=\small, text=TextWhite] {No} (next);
            \draw [arrow] (next) |- (scan);
            \draw [arrow] (exploit) -- (payload);
            \draw [arrow] (payload) -- (execute);
            \draw [arrow] (execute) -- (connect);
        \end{tikzpicture}
        
        \vspace{0.8em}
        \begin{minipage}{0.95\linewidth}
            \normalsize
            The network scanning phase (\texttt{scankill.c}) identifies machines that can potentially be infected. After gaining access (e.g., through Telnet brute force or exploiting CVE vulnerabilities), the payload file is downloaded and executed to establish a continual connection back to the C2 server [2].
        \end{minipage}
    }



    \block{\textcolor{AccentOrange}{7.} Security Risk Assessment}{
        \centering
        \normalsize
        \begin{tikzpicture}[
            box/.style={rectangle, draw=none, minimum width=4.5cm, minimum height=2.2cm, align=center, font=\normalsize\bfseries, rounded corners=6pt, drop shadow},
            high/.style={fill=AccentRed!80, text=DarkBackground},
            med/.style={fill=AccentOrange!80, text=DarkBackground},
            low/.style={fill=AccentGreen!80, text=DarkBackground}
        ]
            \node[box, high] (topleft) at (0,0) {Hardcoded C\&C\\(Single Point\\of Failure)};
            \node[box, med, right=0.5cm of topleft] (topright) {Plain Text\\Communication\\(Traffic Analysis)};
            \node[box, med, below=0.5cm of topleft] (bottomleft) {Fixed Port\\Usage (1994)\\ (Filtering)};
            \node[box, low, right=0.5cm of bottomleft] (bottomright) {Predictable\\Binary Names\\(Signatures)};
        \end{tikzpicture}
        
        \vspace{0.5em}
        \begin{minipage}{0.95\linewidth}
            \normalsize
            \textbf{High Risk:} Hard-coded IP addresses allow defenders to quickly track the entire botnet down (sinkhole).
            \textbf{Medium Risk:} Unencrypted traffic can be easily inspected and signatures developed.
        \end{minipage}
        \vspace{2.5cm}
    }

    \block{\textcolor{AccentOrange}{8.} Multi-Architecture Support}{
        \centering
        \normalsize
        \renewcommand{\arraystretch}{1.4}
        \begin{tabular}{>{\color{AccentCyan}\bfseries}l >{\color{AccentPurple}\ttfamily}l >{\color{TextWhite}\small}l}
            \toprule
            \color{AccentBlue}\textbf{Architecture} & \color{AccentBlue}\textbf{Binary} & \color{AccentBlue}\textbf{Target Devices} \\
            \midrule
            MIPS & 8mips8 & Router firmware, embedded systems \\
            MIPSEL & 8mpsl8 & Little-endian MIPS devices \\
            SH4 & 8sh48 & SuperH architecture devices \\
            i586 & 8i5868 & Legacy x86 systems \\
            ARMv6l & 8arm68 & ARM-based IoT devices \\
            i686 & 8i6868 & Modern x86 systems \\
            PowerPC & 8ppc8 & PowerPC-based systems \\
            M68k & 8m68k8 & Motorola 68000 systems \\
            SPARC & 8spc8 & SPARC architecture systems \\
            ARMv4l & 8arm48 & Older ARM devices \\
            ARMv5l & 8arm58 & ARMv5 systems \\
            ARMv7l & 8arm78 & Modern ARM devices \\
            \bottomrule
        \end{tabular}

        \vspace{0.8em}
        \colorbox{BlockBackground}{\parbox{0.9\linewidth}{\centering\normalsize
        \textbf{\color{AccentCyan}12 architectures supported} - Massive IoT attack surface
        }}
        
        \vspace{0.5em}
        \begin{minipage}{0.95\linewidth}
            \small
            In contrast to earlier IoT botnets, SlumX has 12 separate device types (architectures) which increases the number of possible infected devices [3][5].
        \end{minipage}
    }

    \column{0.33}

    \vspace{-8cm}
    \block{\textcolor{AccentOrange}{9.} Attack Vector Complexity}{
        \centering
        \begin{tikzpicture}[scale=1.1, transform shape]
        \begin{axis}[
            xlabel={\textbf{\color{TextWhite}Attack Type}},
            ylabel={\textbf{\color{TextWhite}Lines of Code}},
            symbolic x coords={UDP,TCP,HTTP,HOLD,JUNK,CNC,STD},
            xtick=data,
            ybar,
            bar width=35pt,
            grid=major,
            grid style={line width=0.5pt, draw=gray!50},
            width=16cm,
            height=12cm,
            x tick label style={rotate=45, anchor=east, font=\normalsize, color=TextWhite},
            y tick label style={font=\normalsize, color=TextWhite},
            axis line style={line width=2pt, draw=AccentBlue},
            tick style={line width=2pt, draw=AccentBlue},
            label style={font=\large, text=TextWhite},
            every axis plot/.append style={fill=AccentPurple!80, draw=AccentPurple, line width=2pt, drop shadow}
        ]
        \addplot coordinates {(UDP,73) (TCP,89) (HTTP,95) (HOLD,67) (JUNK,42) (CNC,51) (STD,35)};
        \end{axis}
        \end{tikzpicture}
        
        \vspace{0.5em}
        {\normalsize\color{TextWhite} HTTP-based attacks show highest complexity (95 LOC).}
        
        \vspace{1em}
        \begin{minipage}{0.95\linewidth}
            \normalsize
            The botnet provides a flexible array of possibilities for DDoS. From analyzing the codebase, we note that the most complex vector is HTTP Flooding, which involves building valid headers in order to evade standard filters (95 LOC); whereas both UDP Floods and TCP Floods produce extremely high amounts of roughed up traffic through raw direct socket manipulations.
        \end{minipage}
    }

    \block{\textcolor{AccentOrange}{10.} Key Findings \& Conclusions}{
        \normalsize
        \begin{itemize}\setlength\itemsep{0.6em}
            \item \textbf{\color{AccentCyan}Multiple Architectures:} Twelve different processor architecture families (including MIPS, ARM, and x86) are supported by the implementation of extensive cross-platform support for the IoT. This support increases the number of individuals that can be infected by malware.
            \item \textbf{\color{AccentPurple}Advanced Evasion:} IoT-centric defensive features have been incorporated into the malware, including anti-debugging functions and the use of dynamic process masquerading.
            \item \textbf{\color{AccentOrange}Targeted Exploitation:} The new strategy for launching attacks against IoT systems is to focus on narrow, high-precision attacks targeted at specific manufacturers, rather than broad-based, indiscriminate attacks based on mass scanning.
            \item \textbf{\color{AccentRed}The Future:} Increasing sophistication through Mirai-Driven Botnets [4].
        \end{itemize}

        \vspace{1em}
        \colorbox{BlockBackground}{\parbox{0.96\linewidth}{\normalsize\color{AccentCyan}
        \textbf{Future Research:}
        \vspace{0.2em}
        \begin{itemize}\setlength\itemsep{0.3em}
            \item Dynamic analysis frameworks
            \item Specialized detection technologies [4]
        \end{itemize}
        }}
    }

    \block{References}{
        \normalsize
        \begin{itemize}\setlength\itemsep{0.4em}
            \item[1] M. Antonakakis et al., "Understanding the Mirai Botnet," \emph{USENIX Security}, 2017.
            \item[2] A. Marzano et al., "The evolution of bashlite and mirai IoT botnets," \emph{IEEE S\&P}, 2018.
            \item[3] G. Vishwakarma and G. S. Chhabra, "Targeted DDoS Attacks on IoT: A Survey of the Mirai Botnet and its Variants," \emph{AISC}, 2023.
            \item[4] A. D. Alghazzawi et al., "An Efficient Hybrid Deep Learning Model for Detecting IoT Botnet Attacks," \emph{IEEE Access}, 2024.
            \item[5] Statista, "IoT connected devices installed base worldwide," 2024.
        \end{itemize}
        
        \vspace{1em}
        \begin{center}
        \textbf{\large\color{AccentBlue}Research Paper Access}\\
        \vspace{0.5em}
        \includegraphics[width=8cm]{QR_research.png}
        \end{center}
        \vspace{0.5cm}
    }

\end{columns}

% Footer profesional
\block{}{
    \vspace{0.3cm}
    \centering
    \large
    \textcolor{AccentCyan}{\textbf{Contact:}} \href{mailto:armelopa@uide.edu.ec}{\textcolor{TextWhite}{armelopa@uide.edu.ec}} 
    \hspace{2cm}
    \textcolor{AccentPurple}{\textbf{Repository:}} \href{https://github.com/mat1520/slumpx}{\textcolor{TextWhite}{github.com/mat1520/slumpx}}
    \hspace{2cm}
    \textcolor{AccentOrange}{\textbf{Date:}} \textcolor{TextWhite}{December 2025}
    \vspace{0.3cm}
}

\end{document}
